...
for epoch in range(epochs):
  ...
  f1_macro_C_eval = f1_score(eval_dataframe['Tag_C'].values.tolist(), preds_C_eval, average='macro')
  all_f1.append(f1_macro_C_eval)
  if f1_macro_C_eval > best_f1:
    best_f1 = f1_macro_C_eval
    best_preds = preds_C_eval
    save_model(epoch + 1, sexist_model, optimizer, scheduler)

  print(f'EPOCH [{epoch + 1}/{epochs}] | Current F1-Macro {round(f1_macro_C_eval * 100, 2)}')
  print(f'EPOCH [{epoch + 1}/{epochs}] | Best F1-Macro {round(best_f1 * 100, 2)}')
  print(confusion_matrix(eval_dataframe['Tag_C'].values.tolist(), preds_C_eval))
...
test_preds_names = [vector_label_map[label_id] for label_id in preds_B_test]
test_B['label_pred'] = test_preds_names
test_B.to_csv('test_C_preds.csv')
